NG_DOCS={
  "sections": {
    "api": "API Reference"
  },
  "pages": [
    {
      "section": "api",
      "id": "pipSettings.Service:pipSettings",
      "shortName": "pipSettings",
      "type": "service",
      "moduleName": "pipSettings",
      "shortDescription": "Service provides an interface to manage &#39;Settings&#39; component behaviour.",
      "keywords": "abstract adding api app behaviour chose collection component config configuration default establish established interface manage menu method object opened passed phases pipsettings register registered returns service settings tab tabobj tabs transeferred validates"
    },
    {
      "section": "api",
      "id": "pipSettings.Service:pipSettingsProvider",
      "shortName": "pipSettingsProvider",
      "type": "service",
      "moduleName": "pipSettings",
      "shortDescription": "Service provides an interface to manage &#39;Settings&#39; component behaviour.",
      "keywords": "abstract adding api app behaviour chose collection component config configuration default establish established interface manage menu method object opened passed phases pipsettings pipsettingsprovider register registered returns service settings tab tabobj tabs transeferred validates"
    },
    {
      "section": "api",
      "id": "pipSettings.Tab:pipSettingsTabController",
      "shortName": "pipSettingsTabController",
      "type": "controller",
      "moduleName": "pipSettings",
      "shortDescription": "The controller is used for the whole settings tabs and provides",
      "keywords": "active_sessions adapt api authorization base_info changes component configuration content controller header integrated left load menu method mobile navigation object panel pipappbar pipsettings predefined require screens selected settings tab tabs transfer user"
    },
    {
      "section": "api",
      "id": "pipUserSettings.BasicInfo:pipUserSettingsBasicInfoController",
      "shortName": "pipUserSettingsBasicInfoController",
      "type": "controller",
      "moduleName": "pipUserSettings",
      "shortDescription": "Controller for the predefined &#39;basic_info&#39; state.",
      "keywords": "$rootscope api basic_info basicinfo change changes controller dialog event exit input method object opens panel party password pipusersettings pipusersettingsbasicinfocontroller predefined profile remote responses saved saves server sync triggered updated updates user"
    },
    {
      "section": "api",
      "id": "pipUserSettings.ChangePassword:pipUserSettingsChangePasswordController",
      "shortName": "pipUserSettingsChangePasswordController",
      "type": "controller",
      "moduleName": "pipUserSettings",
      "shortDescription": "Controller for dialog panel of password change.",
      "keywords": "api approves change changepassword closes controller dialog fields method opened panel password pipusersettings pipusersettingschangepasswordcontroller request sends server typed validates"
    },
    {
      "section": "api",
      "id": "pipUserSettings.Data:pipUserSettingsTabData",
      "shortName": "pipUserSettingsTabData",
      "type": "service",
      "moduleName": "pipUserSettings",
      "shortDescription": "Service reproduces a data layer for settings component.",
      "keywords": "activities api application block cancels change collection component configuration contact contacts contant corresponded data email entity errorcallback events function interact interface invokes item layer method needed object parameters party passed pervious pipdatamodel pipusersettings pipusersettingstabdataprovider position previous process profile property record reftype remove removes removing reproduces retrieves saves server service session settings start successcallback transaction updated updates updating user verification verified verifies verifydata"
    },
    {
      "section": "api",
      "id": "pipUserSettings.Data:pipUserSettingsTabDataProvider",
      "shortName": "pipUserSettingsTabDataProvider",
      "type": "service",
      "moduleName": "pipUserSettings",
      "shortDescription": "Service reproduces a data layer for settings component.",
      "keywords": "active activities api blocks collection component contacts current data interact interface layer method object party pipdatamodel pipusersettings pipusersettingstabdataprovider promise reproduces request resolver retrieve retrieves server service session sessions settings user"
    },
    {
      "section": "api",
      "id": "pipUserSettings.Sessions:pipUserSettingsSessionsController",
      "shortName": "pipUserSettingsSessionsController",
      "type": "controller",
      "moduleName": "pipUserSettings",
      "shortDescription": "Controller provides an interface for managing active sessions.",
      "keywords": "active api closes configuration controller interface managing method object passed pipusersettings pipusersettingssessionscontroller session sessions"
    },
    {
      "section": "api",
      "id": "pipUserSettings.VerifyEmail:pipUserSettingsVerifyEmailController",
      "shortName": "pipUserSettingsVerifyEmailController",
      "type": "controller",
      "moduleName": "pipUserSettings",
      "shortDescription": "Controller for verify email dialog panel.",
      "keywords": "aborts api closes controller dialog email entered initiates method opened panel pipusersettings pipusersettingsverifyemailcontroller request sends server verify verifyemail"
    }
  ],
  "apis": {
    "api": true
  },
  "__file": "_FAKE_DEST_/js/docs-setup.js",
  "__options": {
    "startPage": "/api",
    "scripts": [
      "https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.3.20/angular.min.js",
      "https://cdnjs.cloudflare.com/ajax/libs/marked/0.3.5/marked.js",
      "https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.3.20/angular-animate.min.js"
    ],
    "styles": [],
    "title": "API Reference",
    "html5Mode": true,
    "editExample": true,
    "navTemplate": false,
    "navContent": "",
    "navTemplateData": {},
    "loadDefaults": {
      "angular": false,
      "angularAnimate": false,
      "marked": false
    }
  },
  "html5Mode": true,
  "editExample": true,
  "startPage": "/api",
  "scripts": [
    "https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.3.20/angular.min.js",
    "https://cdnjs.cloudflare.com/ajax/libs/marked/0.3.5/marked.js",
    "https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.3.20/angular-animate.min.js"
  ]
};